...
// ============================================================
// CARESYNC AI SERVICE
// Comprehensive medical knowledge base with local fallback
// ============================================================
...
const MEDICAL_DISCLAIMER =
  'I can provide general health information and help organize your medical information, but I cannot provide a medical diagnosis. Please consult a qualified healthcare professional for diagnosis and treatment.';

const EMERGENCY_WARNING =
  '⚠️ IMPORTANT: Based on what you describe, this could require urgent medical attention. Please contact emergency services (911) or visit the nearest emergency department immediately. Do not delay seeking professional care.';
...
const MEDICAL_KNOWLEDGE: KnowledgeEntry[] = [
  // ---- VITALS & MEASUREMENTS ----
  {
    patterns: [
...
export const HEALTH_ASSESSMENT_QUESTIONS: { key: string; question: string; translations: Record<string, string> }[] = [
...
function findBestMatch(userMessage: string): KnowledgeEntry | null {
...
function generateGreeting(language: string): string {
...
export async function sendMessage(
  ...
  if (lowerMsg.includes('summarize') && (lowerMsg.includes('report') || lowerMsg.includes('test')) || lowerMsg.includes('latest report') || lowerMsg.includes('my report')) {
    try {
      ...
      if (patient && patient.reports.length > 0) {
        ...
        try {
          if (rep.extractedData) analysisData = JSON.parse(rep.extractedData);
        } catch {}
        ...
        const summaryText = `Here is the AI clinical summary for your latest report (**${rep.title}**):\n\n` +
          ...
          (analysisData?.importantValues ? analysisData.importantValues.slice(0, 5).map((v: any) => `• **${v.parameter}:** ${v.value} ${v.unit} (Ref: ${v.referenceRange}) — *${v.status}*`).join('\n') : `• Documentation recorded: ${rep.title}`) +
          `\n\n**Abnormal / Notable Values:**\n` +
          (analysisData?.abnormalValues && analysisData.abnormalValues.length > 0
            ? analysisData.abnormalValues.map((ab: any) => `⚠️ **${ab.parameter}:** ${ab.value} — ${ab.note}`).join('\n')
  ...
  if (lowerMsg.includes('what reports') || lowerMsg.includes('reports did i share') || lowerMsg.includes('shared reports') || lowerMsg.includes('reports shared')) {
    try {
      ...
      if (patient) {
        const sharedReportsList = patient.reports.map((r, i) =>
          `${i + 1}. **${r.title}** (${r.reportType}) — Shared with Dr. Priya Sharma on ${new Date(r.uploadDate).toLocaleDateString('en-IN')}`
        ).join('\n');
...
export async function generateIntakeSummary(
  ...
  patientId?: string,
  userId?: string
): Promise<{ summary: string; savedToDb: boolean }> {
  const sections = [
 ...
 */
export function getAssessmentQuestions(language: string = 'en') {
  return HEALTH_ASSESSMENT_QUESTIONS.map((q) => ({
    key: q.key,
