...
export async function getPatientDashboard(req: AuthRequest, res: Response) {
  try {
    ...
    const vitalTrends = await prisma.vital.findMany({
      where: { patientId: patient.id },
    ...
    // 6. Wellness Trends
    const wellnessTrends = await prisma.wellnessEntry.findMany({
      where: { patientId: patient.id },
      orderBy: { date: 'asc' },
      take: 7,
    });
...
export async function getMyHealth(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    const patient = await prisma.patient.findFirst({
      where: { userId },
...
export async function getMedicalHistory(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    const patient = await prisma.patient.findFirst({
      where: { userId },
...
export async function getMentalWellness(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    const patient = await prisma.patient.findFirst({
      where: { userId },
...
export async function createMentalWellnessEntry(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
...
export async function getElderlyCare(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
...
export async function notifyCaregiver(req: AuthRequest, res: Response) {
  try {
