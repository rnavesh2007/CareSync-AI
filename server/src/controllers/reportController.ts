...
function generateClinicalAnalysis(reportType: string, title: string, fileName: string, patientName: string = 'Arjun Kumar') {
...
function detectReportType(fileName: string, title: string, declaredType?: string): string {
  if (declaredType && declaredType !== 'Other') return declaredType;
  const combined = `${fileName} ${title}`.toLowerCase();

  if (combined.includes('cbc') || combined.includes('hemogram') || combined.includes('complete blood')) return 'CBC';
  if (combined.includes('hba1c') || combined.includes('glycated')) return 'HbA1c';
  if (combined.includes('sugar') || combined.includes('glucose')) return 'Blood Sugar';
  if (combined.includes('lipid') || combined.includes('cholesterol')) return 'Lipid Profile';
  if (combined.includes('liver') || combined.includes('lft') || combined.includes('bilirubin')) return 'Liver Function';
  if (combined.includes('kidney') || combined.includes('kft') || combined.includes('creatinine') || combined.includes('renal')) return 'Kidney Function';
  if (combined.includes('urine') || combined.includes('urinalysis')) return 'Urine Test';
  if (combined.includes('x-ray') || combined.includes('xray') || combined.includes('radiograph')) return 'X-Ray';
  if (combined.includes('ct') || combined.includes('computed tomography')) return 'CT';
  if (combined.includes('mri')) return 'MRI';
  if (combined.includes('ultrasound') || combined.includes('usg') || combined.includes('sonogram')) return 'Ultrasound';
  if (combined.includes('ecg') || combined.includes('ekg') || combined.includes('electrocardiogram')) return 'ECG';
  if (combined.includes('rx') || combined.includes('prescription')) return 'Prescription';
  if (combined.includes('discharge') || combined.includes('discharge summary')) return 'Discharge Summary';
  if (combined.includes('vaccine') || combined.includes('vaccination')) return 'Vaccination';
  if (combined.includes('radiology')) return 'Radiology';
  if (combined.includes('pathology')) return 'Pathology';
  if (combined.includes('consultation') || combined.includes('doctor note')) return 'Doctor Consultation';
  if (combined.includes('blood')) return 'Blood Test';
