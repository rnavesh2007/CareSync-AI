...
async function getQueueList(req, res) {
    try {
        ...
        if (req.user?.role === 'PATIENT') {
            const patient = await index_js_1.prisma.patient.findFirst({ where: { userId: req.user.id } });
            if (patient) {
...
async function updateQueueStatus(req, res) {
    try {
        ...
        const entry = await index_js_1.prisma.queueEntry.update({
            where: { id },
            data: {
                ...(status && { status }),
                ...(priority && { priority }),
...
async function callNextToken(req, res) {
    try {
        ...
        await index_js_1.prisma.queueEntry.updateMany({
            where: { department, status: 'IN_CONSULTATION' },
            data: { status: 'COMPLETED', estimatedWaitTime: 0 },
        });
        // Find next waiting token
        const nextWaiting = await index_js_1.prisma.queueEntry.findFirst({
        ...
        const called = await index_js_1.prisma.queueEntry.update({
...
async function completeToken(req, res) {
    try {
        const { id } = req.params;
        const entry = await index_js_1.prisma.queueEntry.update({
            where: { id },
...
async function generateQueueToken(req, res) {
    try {
        ...
        const countToday = await index_js_1.prisma.queueEntry.count();
        const tokenNumber = `A${120 + countToday + 1}`;
        const waitingCount = await index_js_1.prisma.queueEntry.count({
            where: { department, status: 'WAITING' },
        });
        const estimatedWaitTime = (waitingCount + 1) * 3;
        const entry = await index_js_1.prisma.queueEntry.create({
            data: {
